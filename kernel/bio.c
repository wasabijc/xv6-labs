...
#include "fs.h"
#include "buf.h"
#define BKSIZE 13 //哈希桶的大小
#define NBUFSIZE NBUF * BSIZE //缓冲区大小
struct {
  struct buf bucket[BKSIZE];//哈希桶
  struct spinlock lock[BKSIZE];//每个桶一把锁
...
binit(void)
{
  ...
  for(int i = 0; i<  BKSIZE;i++){
    ...
    for(b = bcache.buf+NBUF*i; b < bcache.buf+NBUF*(i+1); b++){//遍历第i个缓冲区的所有元素
      //每个桶有NBUF个元素
...
{
  struct buf *b;
  int idx = (dev+blockno) % BKSIZE;//设备号+块号作为key
